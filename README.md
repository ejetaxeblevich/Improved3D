...
```c
   Class I3D
   {
      ...
      [M] table GetAllEntities( bool GetsIntoCamera )         /* Возвращает все объекты на карте, что имеют позиции CVector и их количество: bool GetsIntoCamera = захватывает только объекты, что в полне зрения камеры, если true */
